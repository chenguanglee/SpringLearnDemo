...
public class 机器人的运动范围13 {
    ...
    public int movingCount1(int m, int n, int k){
        isVisit = new boolean[m][n];
        return move1(m,n,0,0,k);
    }

    private int move1(int m,int n,int x, int y ,int k){
        if (x >=m || y >= n || x <0 || y <0 || isVisit[x][y]){
            return 0;
        }
        int i = x / 10 + x % 10;
        int j = y / 10 + y % 10;
        ...
        isVisit[x][y] = true;
        return 1 + move1(m, n, x + 1, y, k) + move1(m, n, x, y + 1, k);
    }

    @Test
    public void test111(){
        int i = movingCount(2, 3, 1);
        int j = movingCount1(2, 3, 1);
        System.out.println(i);
